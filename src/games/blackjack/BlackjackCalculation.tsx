...
export function BlackjackCalculation() {
  ...
  const generateNewChallenge = () => {
    // Ensure values are multiples of 10 and valid
    const validMin = Math.max(10, Math.ceil(minBet / 10) * 10);
    const validMax = Math.max(validMin, Math.floor(maxBet / 10) * 10);
    
    const steps = (validMax - validMin) / 10;
    const randomStep = Math.floor(Math.random() * (steps + 1));
    const randomNum = validMin + (randomStep * 10);
  ...
  const calculateBlackjack = (num: number): number => {
    return num * 1.5;
  };

  const checkAnswer = () => {
    const correctAnswer = calculateBlackjack(targetNumber);
  ...
  const handleMinChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Don't force valid range immediately to allow typing, but logic in generate handles it
    setMinBet(val);
  };
    ...
    <div className="space-y-4 sm:space-y-6">
      <div className="bg-black/30 backdrop-blur-sm rounded-xl p-4 sm:p-6 border border-white/10">
        <div className="flex justify-between items-start mb-4">
          <div>
            <h2 className="text-xl sm:text-2xl font-bold text-white mb-1">Calcul de Blackjack</h2>
            <p className="text-sm sm:text-base text-emerald-200">
              Calculez rapidement le paiement Blackjack (mise × 1,5)
            </p>
          </div>
          <button
        ...
        <div className="max-w-md mx-auto">
          <div className="mb-6">
            <label className="block text-white font-medium mb-3 text-sm sm:text-base">
              Quel est le paiement Blackjack ?
            </label>
              ...
              {showAnswer && (
                <div className="mb-4 p-4 bg-black/30 rounded text-center">
                  ...
                  <div className="text-white font-mono text-2xl">
                    {targetNumber} × 1,5 = {calculateBlackjack(targetNumber)}
                  </div>
                </div>
              )}
